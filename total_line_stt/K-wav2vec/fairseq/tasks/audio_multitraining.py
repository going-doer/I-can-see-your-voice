...
from fairseq.data import AddMultiDataset, Dictionary, FileAudioDataset, encoders
...
@dataclass
class AudioMultitrainingConfig(FairseqDataclass):
    ...
    del_silence: bool = field(
        default=False,
        metadata={
            "help": "required for del_silence "
            "adds 'prev_output_tokens' to input and appends eos to target"
        },
    )


@register_task("audio_multitraining", dataclass=AudioMultitrainingConfig)
class AudioMultitrainingTask(FairseqTask):
    """"""
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if task_cfg.labels:
            ...
            add_labels = []
            with open(add_label_path, "r", encoding="utf-8") as f:
                add_labels = [
                    line for i, line in enumerate(f)
                    if i in self.datasets[split].line_inds
                ]
