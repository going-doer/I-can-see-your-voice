...
## 평가를 위해서 ClovaCall Dictionary를 위해서 등록되지 않는 토큰을 넣어줍니다.
# (모델을 활용하여 토큰을 다 생성한 후에 target label에 대해서한 해당 토큰을 사용합니다.)
...
def process_predictions2(
        args, hypos, res_files, id
):
    
    hyp_unit_lines, hyp_word_lines = [], []
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        ...
        if res_files is not None:
            ...
            unit_line = "{}\t{}".format(id, " ".join(hyp_piece.replace(' ',"|")+"|"))
            print(
                unit_line,
                file=res_files["hypo.units"],
            )
            hyp_unit_lines.append(unit_line)

            word_line = "{}\t{}".format(id, hyp_word)
            print(
                word_line,
            ...
            print(
                word_line,
                file=res_files["kw2v.words"],
            )

            

            hyp_word_lines.append(word_line)
    return {
        'hyp_unit_lines': hyp_unit_lines,
        'hyp_word_lines': hyp_word_lines,
    }    
...
def prepare_result_files2(args):
    def get_res_file(file_prefix):
        path = os.path.join(
            args.results_path,
            "{}-{}-{}.txt".format(
    ...
    def get_res_csv_file(file_prefix):
        path = os.path.join(
            args.results_path,
            "{}-{}.csv".format(
                file_prefix, args.gen_subset
            ),
        )
        return open(path, "w", buffering=1, encoding='utf-8')
    ...
    return {
        "hypo.words": get_res_file("hypo.word"),
        "hypo.units": get_res_file("hypo.units"),
        "kw2v.words": get_res_csv_file("kw2v")
    }    
...
def main(args):
    ...
    models, saved_cfg = checkpoint_utils.load_model_ensemble(
        ...
        #arg_overrides=ast.literal_eval(args.model_overrides), # 민주 이거 이용해서 하면 됨. !
        arg_overrides=ast.literal_eval(args.model_overrides), # 민주 추가
        task=task,
    ...
    ## 민주
    ## dev.tsv을 통해서 경로 가져오기
    dev_tsv_path = os.path.join(args.data, "dev.tsv")
    dev_dict = {}    
    with open(dev_tsv_path, "r", encoding='utf-8') as file:
        cnt = 0
        wav_dir_path = line = file.readline().strip()
        while True:
            line = file.readline()
            if line == '':
                break
            line_arr = str(line).strip().split("\t")
            dev_dict[cnt] = str(os.path.join(wav_dir_path, line_arr[0]))
            cnt += 1
    ...
    res_files = prepare_result_files2(args)
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                ...
                # id = sample_id
                id = dev_dict[sample_id]
                
                postprocess_result = process_predictions2(
                    args,
                    hypos[i],
                    res_files,
                    id,
