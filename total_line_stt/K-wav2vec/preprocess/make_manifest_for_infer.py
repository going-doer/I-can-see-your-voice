...
def save_dict(args, transcripts, dict_name='dict.ltr.txt', alphabet_name='alphabet.txt'):
    ...
    with open(os.path.join(args.dest, dict_name), 'w', encoding="utf-8") as write_f:
        for idx, (grpm, freq) in enumerate(zip(vocab_list, vocab_freq)):
            print("{} {}".format(grpm, freq), file=write_f)

    ## Write Vocab files
    with open(os.path.join(args.dest, alphabet_name), 'w', encoding='utf-8') as write_f:
...
def save_lexicon(args, texts, lexicon_name='lexicon.lst'):
    ...
    with open(os.path.join(args.dest, lexicon_name), 'w', encoding='utf-8') as write_f:
...
def save_files(args, file_name, dir_path, fileinfo, texts, transcripts):
    with open(os.path.join(args.dest, file_name + ".tsv"), 'w', encoding="utf-8") as tsv_out, open(
            os.path.join(args.dest, file_name + ".ltr"), "w", encoding="utf-8"
    ) as ltr_out, open(
        os.path.join(args.dest, file_name + ".wrd"), "w", encoding="utf-8"
    ) as wrd_out:
...
def load_script(args, script_path, info_data, token_limit=sys.maxsize):
    ...
    with open(script_path, "r", encoding="utf-8") as f:
        for line in tqdm(f):
            ...
            file_path = os.path.join(args.root, items[0].strip())
            file_path = os.path.realpath(file_path)
            # 민주
            if len(Path(file_path).stem.split("_")) > 1:
